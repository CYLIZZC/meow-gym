...
async function initPayment() {
    // 1. 取得 form 資料
    const formData = {
        MerchantID: document.querySelector('[name="MerchantID"]').value,
        MerchantTradeNo: document.querySelector('[name="MerchantTradeNo"]').value,
        MerchantTradeDate: document.querySelector('[name="MerchantTradeDate"]').value,
        PaymentType: document.querySelector('[name="PaymentType"]').value,
        TotalAmount: document.querySelector('[name="TotalAmount"]').value,
        TradeDesc: document.querySelector('[name="TradeDesc"]').value,
        ItemName: document.querySelector('[name="ItemName"]').value,
        ReturnURL: document.querySelector('[name="ReturnURL"]').value,
        ChoosePayment: document.querySelector('[name="ChoosePayment"]').value,
        EncryptType: document.querySelector('[name="EncryptType"]').value
    };

    // 2. 呼叫後端 API 產生 CheckMacValue
    const response = await fetch('/ecpay/generateCheckMac', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
    ...
    // 3. 填入 form
    document.getElementById('CheckMacValue').value = data.CheckMacValue;
